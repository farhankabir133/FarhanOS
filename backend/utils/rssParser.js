export function parseMediumRSS(xmlText) {
    ...
    return items.map((match, idx) => {
        ...
        if (lowercaseCategories.some((c) => c.includes("ai") ||
            c.includes("artificial") ||
            c.includes("gpt") ||
            c.includes("llm"))) {
            finalCategory = "AI";
        }
        else if (lowercaseCategories.some((c) => c.includes("dev") ||
            c.includes("coding") ||
            c.includes("program") ||
            c.includes("software") ||
        ...
        else if (lowercaseCategories.some((c) => c.includes("productiv") ||
            c.includes("work") ||
            c.includes("career") ||
            c.includes("growth"))) {
        ...
        const guidIdMatch = rawGuid.match(/\/p\/([a-f0-9]+)$/) ||
            link.match(/-([a-f0-9]+)$/) ||
