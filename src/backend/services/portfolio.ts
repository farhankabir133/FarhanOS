...
import type { MediumStory, GithubRepo } from "../types/index.js";

const MEDIUM_TTL = 15 * 60 * 1000;
const GITHUB_TTL = 60 * 60 * 1000;

export async function fetchMediumStories(cached: boolean = true): Promise<MediumStory[]> {
  if (cached) {
    const cachedData = cache.get<MediumStory[]>("medium-stories");
...
export async function fetchGithubRepos(cached: boolean = true): Promise<GithubRepo[]> {
  if (cached) {
    const cachedData = cache.get<GithubRepo[]>("github-repos");
    if (cachedData) return cachedData;
  }
