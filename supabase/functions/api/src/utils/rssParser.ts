...
export function parseMediumRSS(xmlText: string): MediumStory[] {
  ...
  return items.map((match, idx) => {
    ...
    if (descMatch) {
      ...
      const snippetMatch = descHtml.match(
        /<p class="medium-feed-snippet">([\s\S]*?)<\/p>/
      );
      if (snippetMatch) {
    ...
    if (
      ...
      lowercaseCategories.some(
        (c) => c.includes("research") || c.includes("science") || c.includes("clinic")
      )
    ) {
      finalCategory = "Research";
    } else if (
      lowercaseCategories.some(
        (c) => c.includes("design") || c.includes("ux") || c.includes("ui")
      )
    ) {
      ...
      lowercaseCategories.some(
        (c) =>
          c.includes("startup") || c.includes("business") || c.includes("saas")
      )
    ) {
      finalCategory = "Startups";
    } else if (
      lowercaseCategories.some(
        (c) => c.includes("philosoph") || c.includes("think")
      )
    ) {
